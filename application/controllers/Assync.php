			...
			$status = ($_POST["kurir"] == "cod") ? 1 : $status;
				...
				if($wa == null){
					...
					$pesan = "
						...
						<a href='".site_url("blackexpo")."'>Klik Disini</a>
					";
	...
	public function lacakiriman(){
		if(isset($_GET["orderid"])){
			...
			curl_setopt_array($curl, array(
				...
				CURLOPT_POSTFIELDS => 'kurir=jnt&resi=JT3791763025',
			...
			if ($err) {
				...
				if($response->data->found === true){
					$respon = $response->data->detail->history;
					if($response->data->detail->status === "DELIVERED"){
						echo "
							<div class='m-b-30'>
								Status: <b style='color:#28a745;'>PAKET TELAH DITERIMA</b><br/>
								Penerima: <b>".strtoupper(strtolower($response->data->detail->receiver))."</b><br/>
								Tgl diterima: ".$this->func->ubahTgl("d M Y H:i",$response->data->detail->date_received)." WIB
							</div>
						";
					}else{
					...
					for($i=0; $i<count($respon); $i++){
						...
						echo "
							<div class='row p-tb-10' style='border-bottom: 1px dashed #ccc;'>
								<div class='col-md-3'>
									<i>".$this->func->ubahTgl("d/m/Y H:i",$respon[$i]->time)." WIB</i>
								</div>
								<div class='col-md-9'>
									<i>".$respon[$i]->desc."</i>
									<i>".$respon[$i]->position."</i>
								</div>
							</div>
						";
					}
